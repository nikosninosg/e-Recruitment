...
diarkeia_leptwn int NOT NULL,
sxolia TEXT NULL,
job_id int(4) NOT NULL,
personality ENUM("1","2","3","4","5"),
CONSTRAINT CON_INTER FOREIGN KEY (usrname) REFERENCES candidate(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT CON_JOB_ID FOREIGN KEY (job_id) REFERENCES applies(job_id) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB; 

insert into interview (usrname, startdate , diarkeia_leptwn, sxolia , job_id, personality) values 
('cleogeo','2017-02-13 12:23:34','20','sxolia',2,'1'),
('zazahir23','2017-09-16 17:03:0' ,'74','sxolia',12,'2'),
('lionarF','2017-08-29 18:12:39', '90','sxolia',6,'3'),
('liagourma','2018-11-09 11:23:45','100','sxolia',10,'4'),
('mnikol','2018-07-20 20:10:20','89','sxolia',11,'5'),
('abrown','2018-06-17 14:08:23','140','sxolia',1,'4')
;

#score
CREATE TABLE vathmologia(
usr_vath varchar(12) NOT NULL,
personality ENUM("1","2","3","4","5"),
education TEXT,
experience TEXT,
job_id int(4) NOT NULL,
CONSTRAINT CON1_VATH FOREIGN KEY (usr_vath) REFERENCES interview(usrname) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT CON2_VATH FOREIGN KEY (job_id) REFERENCES interview(job_id) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;

insert into vathmologia (usr_vath, personality , education, experience,job_id) values 
('cleogeo','1','edu1','exp1',2),
('zazahir23','2','edu2','exp2',12),
('lionarF','3','edu3','exp3',6),
('liagourma','4','edu4','exp4',10),
('mnikol','5','edu5','exp5',11),
('abrown','4','edu6','ecp6',1)
;


/*TOMEIS*/
CREATE TABLE sector(
afm_et CHAR(9) NOT NULL,
titlos VARCHAR(35) NOT NULL,
keimeno TEXT,
ierarxies ENUM('Data Bases', 'Operating Systems'),
upotomis_1ou_ep CHAR(40),
upotomis_2ou_ep CHAR(40),
belongs VARCHAR(35) NULL,
PRIMARY KEY (titlos,afm_et),
CONSTRAINT CON_SECT2 FOREIGN KEY (afm_et) REFERENCES etaireia(AFM) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT CON_SECT1 FOREIGN KEY (belongs) REFERENCES sector(titlos) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;

#
insert into sector(afm_et, titlos, keimeno,ierarxies,upotomis_1ou_ep,upotomis_2ou_ep, belongs) values
('123432211','Computer Engineering','keimeno..','Data Bases','MariaDB','Queries',null),
('023453344','Engineering','keimeno..','Data Bases','MySQL','Triggers','Computer Engineering'),
('23122345','Computer Science','keimeno..','Operating Systems','Windows','GUI',null),
('18765549','Engineering','keimeno..','Operating Systems','Linux','Bash shell','Computer Science')
;
...
ektelesi ENUM('epitixis','anepitixis') NOT NULL,
eidos_entolis ENUM('insert','update','delete') NOT NULL,
onoma_pinaka CHAR(20) NOT NULL,
#PRIMARY KEY (usrname),
CONSTRAINT CON_IST1 FOREIGN KEY (usrname) REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;

###
-- CANDIDATE --
insert into istoriko (usrname, imerominia , ektelesi , eidos_entolis, onoma_pinaka) values 
('cleogeo', '2018-02-13 12:23:34', 'epitixis', 'insert', 'candidate'),
('zazahir23', '2017-05-11 14:08:23', 'anepitixis', 'update', 'candidate'),
('lionarF', '2018-10-07 20:09:10', 'epitixis', 'delete', 'candidate'),
('liagourma', '2018-05-22 17:03:0', 'anepitixis', 'insert', 'candidate'),
('mnikol', '2017-11-08 21:07:12', 'epitixis', 'update', 'candidate'),
('abrown', '2018-01-27 16:02:56', 'anepitixis', 'delete', 'candidate');

-- RECRUITER --
insert into istoriko (usrname, imerominia , ektelesi , eidos_entolis, onoma_pinaka) values 
('msmith', '2017-06-23 13:12:34', 'epitixis', 'insert', 'recruiter'),
('varcon82', '2018-12-03 18:12:39', 'anepitixis', 'update', 'recruiter'),
('bettyg', '2017-04-12 12:23:10', 'epitixis', 'delete', 'recruiter'),
('papad', '2018-11-17 23:10:08', 'anepitixis', 'insert', 'recruiter'),
('n_tri', '2018-03-08 20:10:20', 'epitixis', 'update', 'recruiter'),
('Giankost', '2018-02-23 11:08:15', 'anepitixis', 'delete', 'recruiter'),
('pavkie', '2017-03-25 11:23:45', 'epitixis', 'insert', 'recruiter');
